...
const network = new Network<BlogInfo>(); // we now essentially have "readers", "latestReader", and "likes" publishers

const unsub = network.subscribe("readers", (people) => {
...
network.publish("readers", [{ name: "Evyatar", age: 19 }]);
...
const [person] = store.get(); // the store's state is returned as an array; you can read the API below to see why
...
// data, just like a store's, is returned as an array
